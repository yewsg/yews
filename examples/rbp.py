...
# original cpic outputs
model = cpic_v1(pretrained=False)
with open('cpic_original.pickle', 'rb') as f:
...
# fine-tuned original cpic outputs
model = cpic_v1(pretrained=False)
with open('cpic_original_finetuned.pickle', 'rb') as f:
    state_dict = pickle.load(f)
model.load_state_dict(state_dict)
model.eval()
...
ax.grid(True)
ax.legend()
# simplified cpic outputs
model = cpic_v2(pretrained=False)
with open('cpic_simplified.pickle', 'rb') as f:
    state_dict = pickle.load(f)
model.load_state_dict(state_dict)
model.eval()
...
# fine-tuned simplified cpic outputs
model = cpic_v2(pretrained=False)
with open('cpic_simplified_finetuned.pickle', 'rb') as f:
    state_dict = pickle.load(f)
model.load_state_dict(state_dict)
model.eval()
